...
class GameInstance:
    def __init__(self, name, clients):
        ...
        #Gun sprites------------------------------------------------------------------------------------------------
        #               Gun(Gun name,     gun bullet,              damage,          weapon image,        spread,   fire rate)
    ...
    def check_damage(self):
        ...
        while self.running:
            ...
            players = dict(zip(name,obj))#To prevent dictionary error
            for name, obj in players.items(): #Loop through each player
                try:
                    ...
                    #If the player is outside of the storm and the time for taking damage is more than 1 second
                ...
                for b in obj.bullets: #Loop through each of their bullets
                    for p in [i for i in self.player_dict.values()]: #Loop through the players
                        ...
                        for ix, iy in interpolate:#Checks everything in between +20ish pixels (to prevent bullet from skipping over due to lag)
    ...
    def storm(self,start = False):
        if start:#Starting storm for the first time
            self.storm_time = t.time()
        else:
            while self.running:
                #Every 60 seconds, switch storm to idle and then to moving
                if t.time()-self.storm_time>60:
                    self.storm_time = t.time()
                    if self.storm_next == "idle":
                        self.storm_next = "moving"
                        self.storm_rad[self.storm_state]
                        #The continuous change in which the storm will be moving
                        ...
                        #Sets storm back to idle
                        self.storm_next = "idle"
                        #Prevent storm state from going over the list
                ...
                if t.time()-self.storm_moving>.1 and self.storm_next == "moving":#Storm updates every .1 seconds
                    self.storm_moving = t.time()
                    #Moves the storm accordingly
